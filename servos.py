...
class Robot:
    ...
    def calibrateSpeeds(self):
        ...
        while currentpwm >= 1.3:
            ...
            time.sleep(2)
            speedtuple = self.getSpeeds(self.getCounts(), 2)
            ...
            if leftrps < self.leftminrps and leftrps != 0:
                ...
                self.leftminips = self.leftminrps * CIRCUMFERENCE
            ...
            currentpwm = round(currentpwm - 0.01, 2)            
    ...
    def setSpeedsRPS(self, rpsLeft, rpsRight):
        ...
        lastRPS = 0
        for tuple in self.leftmap:
            if tuple[1] >= 1.5 and rpsLeft <= lastRPS and rpsLeft >= tuple[0]:
                if tuple[0] == 0 and lastRPS == 0:
                    pwmLeft = 0
                else:
                    pwmLeft = (lastPWM * (tuple[0] - rpsLeft) + tuple[1] * (rpsLeft - lastRPS)) / (tuple[0] - lastRPS)
            lastRPS = tuple[0]
            lastPWM = tuple[1]
        lastRPS = 0
        lastPWM = 0
        for tuple in self.rightmap:
            if tuple[1] <= 1.5 and rpsRight >= lastRPS and rpsRight <= tuple[0]:
                if tuple[0] == 0 and lastRPS == 0:
                    pwmRight = 0
                else:
                    pwmRight = (lastPWM * (tuple[0] - rpsRight) + tuple[1] * (rpsRight - lastRPS)) / (tuple[0] - lastRPS)
            lastRPS = tuple[0]
            lastPWM = tuple[1]
        lastRPS = 0
        lastPWM = 0
